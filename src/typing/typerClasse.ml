					...
					let l_env = 
						if List.length vars > 0 then
							List.fold_left (fun env v -> ajoute_l_env_var v env) empty_env vars
                                        ...
                                        let g_env = ajoute_g_env_class c_name vars (List.map (fun (Ast.TypeDeclaration (Name (f,_,_), [], [], t_list)) -> (f, vars, [(c_name, List.map (fun v -> Tvar v) vars)], List.map (bf g_env l_env) t_list)) fonctions) g_env in
